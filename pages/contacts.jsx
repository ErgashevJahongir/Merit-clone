        ...
        <div>
            <div className={style.container}>
                <div className={styless.container}>
                    {data?.map((item, id) => {
                        return (<div className={styless.box} key={id}>
                            <p className={styless.thema}><strong>{item.name}</strong></p>
                            <iframe src={item.mapUrl} width="600" height="450" allowFullScreen="" loading="lazy" referrerPolicy="no-referrer-when-downgrade"></iframe>
                            <p className={styless.text}>
                                <strong>Telefon raqamlar:
                                    {item.phone?.map((phoneItem,id) => <React.Fragment key={id}><br /> {phoneItem}</React.Fragment>)}    </strong>
                            </p>
                        </div>)
                    })}
                </div>
